...
export default function ApparelBrandProductPage({ brand, initialApparelData }: { brand: string, initialApparelData?: unknown[] }) {
  ...
  // Set filter visibility based on device type
  useEffect(() => {
  ...
  const apparels = useMemo(() => {
    ...
    if (apparelData?.apparel) {
      return apparelData.apparel;
    }
    // Fallback to initial data only on page 1
    if (initialApparelData && currentPage === 1) {
      return initialApparelData;
    }
    return [];
  }, [apparelData?.apparel, initialApparelData, currentPage]);
  ...
  const totalPages = useMemo(() => {
    ...
    if (apparels.length === PRODUCTS_PER_PAGE) {
      return currentPage + 1;
    }
